...
class DooPager{
    /**
     * CSS class name for the ul
     * @var string
     */
//     public $mainCss='pagination';
    public $mainCss='pagination';
...
.inactivePrev, .inactiveNext{
    ...
    public function setMainClass($mainCss=''){
    ...
    public function paginate($page, $itemPerPage=0){
        ...
        $this->output = '<ul'.(empty($this->mainCss)?'':' class="'.$this->mainCss.'"').'>';

        if($this->_noNextPrev)
            $this->components['prev_link'] = ($this->currentPage != 1 /*&& $this->totalItem >= $this->maxLength*/) ? "<li class=\"{$this->prevCss}\"><a href=\"{$this->baseUrl}{$prev_page}\">{$this->prevText}</a></li> ":"<li class=\"{$this->inactivePrevCss}\"><a href=\"javascript:;\">{$this->prevText}</a></li> ";//    "<span class=\"{$this->inactivePrevCss}\">{$this->prevText}</span> ";
        else
            $this->output .= ($this->currentPage != 1 /*&& $this->totalItem >= $this->maxLength*/) ? "<li class=\"{$this->prevCss}\"><a href=\"{$this->baseUrl}{$prev_page}\">{$this->prevText}</a></li> ":"<li class=\"{$this->inactivePrevCss}\"><a href=\"javascript:;\">{$this->prevText}</a></li> ";
        ...
        $this->components['nums'] = $this->totalItem;
    ...
    public function showPageSize($sizeArr=null){
        ...
        return "<select data-url=\"{$this->baseUrl}\" style=\"margin-bottom: 22px;\" class=\"{$this->pageSizeCss} j_pagesize\" >$items</select>\n";
    }
