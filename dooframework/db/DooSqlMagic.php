...
class DooSqlMagic {
    ...
    /**
     * check a database commection
     */
    public function checkConnect(){
    	try{
	    	$db_info = $this->pdo->getAttribute(PDO::ATTR_SERVER_INFO); //  erkennung von toten persistenten verbindungen ...
    ...
    public function find($model, $opt=null){
		...
		if (isset($opt['filters']) && is_array($opt['filters'])) {
			...
			foreach ($opt['filters'] as $filter) {
                ...
                $fTableAliasName = $fTableName;
                if(isset($filter['alias'])){
                    $fTableName .= ' '.$filter['alias'];
                    $fTableAliasName = $filter['alias'];
                }
				...
				switch($fmodel_rtype) {
					case 'has_one':
						$sqladd['filter'] .= "{$joinType} {$fTableName} ON {$fTableAliasName}.{$fparams['foreign_key']} = {$model->_table}.{$model->_primarykey} ";
						break;
					case 'belongs_to':
						list($frtype, $ffkey ) = self::relationType($this->map, $fmodel_class, $class_name);
						$sqladd['filter'] .= "{$joinType} {$fTableName} ON {$fTableAliasName}.{$fparams['foreign_key']} = {$model->_table}.{$ffkey['foreign_key']} ";
						break;
					case 'has_many':
						list($fmtype, $fmparams) = self::relationType($this->map, $fmodel_class, $class_name);
						if ($fmtype == 'has_many') {
                            $through = $fparams['through'];
                            $aliasThrough = $fparams['through'];
                            if(isset($fparams['alias'])){
                                $through .= ' '.$fparams['alias'];
                                $aliasThrough = $fparams['alias'];
                            }
							$sqladd['filter'] .= "{$joinType} {$through} ON {$model->_table}.{$model->_primarykey} = {$aliasThrough}.{$fparams['foreign_key']}\n";
							$sqladd['filter'] .= "{$joinType} {$fTableName} ON {$fTableAliasName}.{$fmodel->_primarykey} = {$aliasThrough}.{$fmparams['foreign_key']} ";
						} else {
...
//														echo "<h3>$k2 = $v2</h3>";
    ...
    public function update($model, $opt=NULL){
        ...
        if(isset($model->{$obj['_primarykey']})){
            ...
            if(isset($opt['limit'])){
                $sql ="UPDATE {$obj['_table']} SET {$field_and_value} {$where} LIMIT {$opt['limit']}";
            }else{
                $sql ="UPDATE {$obj['_table']} SET {$field_and_value} {$where}";
            }
        }
