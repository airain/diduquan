...
Doo::loadClassAt('mailer/phpmailer');
...
class SendEmail
{
   ...
   */
	...
	public function clearSendto(){
		$this->_mail->ClearAddresses();
	}

	public function clearRepleyTo(){
	...
	public function setDelivery($type = 'mail'){
		...
		switch($type) {
			...
			case 'smtp':
				...
				break;
			case 'postfix':
				$this->_delivery = Doo::conf()->POSTFIX_DELIVERY;//postfix use smtp
				$this->_mail->IsSMTP();
				$this->_mail->SMTPKeepAlive = true;
				$this->_mail->Host = Doo::conf()->POSTFIX_SMTP_HOST;
				$this->_mail->setFrom(Doo::conf()->POSTFIX_FROM,Doo::conf()->POSTFIX_FROM_NAME);
				if(!empty(Doo::conf()->POSTFIX_SMTP_USERNAME)) {
					$this->_mail->SMTPAuth = true;
					$this->_mail->Username = Doo::conf()->POSTFIX_SMTP_USERNAME;
					$this->_mail->Password = Doo::conf()->POSTFIX_SMTP_PASSWORD;
				}
	
				if(!empty(Doo::conf()->POSTFIX_SMTP_MODE)) {
					$this->_mail->SMTPSecure = Doo::conf()->POSTFIX_SMTP_MODE;
				}
	
				$this->_mail->Port = Doo::conf()->POSTFIX_SMTP_PORT;
