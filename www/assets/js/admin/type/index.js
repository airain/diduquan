define(function(require, exports, module) {
   ...
   var main = function(){
       ...
       $(".m_cr_conf").bind('click', createConfig).confirm({msg:'确定要【这样做】吗？'});
       loadlist();
   };

   function createConfig () {
      $.post('/admin/type/cr_conf',{},function (data) {
          if(0==data.result){
   ...
   var loadlist = function(){
       $("#dlist").load("/admin/type/dlist",condition);
   };

   var remove = function(ids){
       if(!ids){return;}
       $.post("/admin/type/delete",{id:ids},function(data){
           if(data.success){
               ids = ids.split(",");
               for(var i in ids){
