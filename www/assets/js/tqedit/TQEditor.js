...
TQE=function(objId,userConfig)
{
	...
	$.cleanWord=function(force){
		...
		d.body.innerHTML = d.body.innerHTML.replace(new RegExp("\n|\r\n","gi"),"`").replace(/<\!\-\-\[if [a-z0-9 ]+\].*?<\!\[endif\]\-\->/ig,'').replace(/`/gi,"\n");
		cleanTag(['SPAN','A'],true);
		cleanTag(['P','B','I','U','TD','TR','TABLE','DIV','COL','FONT'],false);
		//.replace(/<([a-z][a-z0-9]*)[^>]*>\s*<\/\1>/gi,'')	,'FONT'
		d.body.innerHTML=d.body.innerHTML.replace(/<o:[a-z0-9_]+[^>]*>/gi,'').replace(/<\/o:[a-z0-9_]+>/gi,'').replace(/<w:[a-z0-9_]+[^>]*>/gi,'').replace(/<\/w:[a-z0-9_]+>/gi,'').replace(/<\?xml.+?\/>/gi,'').replace(/(<[a-z]+[^>]+>)/ig,function(m,p1){if(p1.indexOf(':')>0){ p1=p1.replace(/[a-z]+:[a-z0-9]+="[^\"]*"/ig,''); }return p1; });
		cleanTag(['SPAN'],true);
